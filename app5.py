...
model.add_seasonality(name="sunspot_cycle", period=11, fourier_order=6)
model.fit(df)
...
ax.plot(df["ds"], df["y"], label="Actual", color="blue", marker="o", linestyle="None", markersize=5)
...
ax.set_title("📉 Custom Plot: Actual vs. Predicted with Prediction Intervals")
ax.set_xlabel("Year")
...
# ax2.axhline(0, ...)
ax2.plot(merged["ds"], merged["residual"], color="purple", marker=".", linestyle="-", label="Residual")
ax2.axhline(0, color="black", linestyle="--", lw=1)
ax2.set_title("Residual (Actual - Predicted) Time Series")
ax2.set_xlabel("Year")
